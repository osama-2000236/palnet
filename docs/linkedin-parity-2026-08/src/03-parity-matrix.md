...
**The headline finding of the scan:** 33 capabilities are _already in the schema, the enum set, or the API and have no engine or no UI_. That is nearly a quarter of LinkedIn's surface sitting in this repo as committed intent with no behaviour behind it. The fastest route to a LinkedIn-class product here is not to add features — it is to finish the ones the schema already promises, and only then extend.
