...
| 8 | **Craft family key confirmation** — `OCCUPATIONS.md` §6 item 5, still open. Keys are forever; Arabic labels are an i18n edit. *"Needs a tradesperson, not a search engine."* | Standing labels | The current keys ship; a key change later is a data migration | §5.4.1 |
...
| 5 | **The scam scanner cries wolf** and gets ignored | Medium | Severe | Precision on the 40 innocent near-misses is asserted *first* in the test suite; graduated actions mean a match warns before it blocks; only family 1 hard-blocks, and only on job descriptions |
