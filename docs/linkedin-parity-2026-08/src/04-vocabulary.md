...
_Enforcement._ `Invoice.userId` and `Invoice.companyId` are payer references and there is no payee column; adding one requires an ADR. The `check-naming.mjs` ban on `order` in the services domain is the second half of this.

_Why it is permanent._ `project-spec.md` already excluded it. Restating it here matters because §11 (services) is exactly the workstream where somebody will propose adding it, and the reason to refuse is not scope — it is that money movement makes Baydar a financial institution under PMA supervision, which is a different company.
