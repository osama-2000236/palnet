...
| id   | Finding                                                                                                                                                                                                                                                                           | Why left                                                                                                            |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| P2-7 | Expo Router warns on every launch that `_message-thread/useTypingIndicator` and `_premium/parts` are declared routes that do not exist. The type generator reports "92 mobile route files" against 38 real screens.                                                               | Dev-only console noise; no user-visible effect. Needs a routing-convention change, which is wider than this review. |
...
| P3-3 | `apps/mobile/expo-env.d.ts` is listed in `apps/mobile/.gitignore` but tracked, so the ignore is inert. Expo writes it without a trailing newline, prettier adds one — they oscillate. Untracking breaks CI, because `type-check` does not regenerate it (only `expo start` does). | Verified the obvious fix breaks the build; leaving it beats churning it.                                            |
