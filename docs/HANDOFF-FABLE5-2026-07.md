...
- **Drift guards instead of risky refactors:** `packages/config/tailwind-preset.js` stays standalone CJS (Metro loads tailwind config with plain Node — deriving it from ui-tokens would break fresh-clone mobile dev on the unbuilt dist), but `lint:tokens` now fails on any hex there that doesn't exist in ui-tokens source (negative-tested). **Superseded by audit 6 (2026-07-25): the preset and its drift guard are both deleted — NativeWind is gone from mobile, so nothing consumed the mirror.** `USER_PREMIUM_POINTS_PRICE` in `billing/pricing.ts` is the single source for the 500-points premium price; `REDEEM_COSTS` imports it (kills the "must stay in sync" comment pair).
...
- **Bug found (zombie hard-load):** any hard load of an `(app)` page rendered TWO `<main>`s — a visible stuck skeleton over the real (hidden) content. Spawned as its own task from this session; **root-caused and fixed same day in §14 (PR #66)** — rAF starvation in hidden tabs, not hydration.
...
## 14. Hard-load zombie fix (2026-07-19, PR #66)

**Symptom:** every hard load of an `(app)` page rendered two `<main>`s — the `(app)/loading.tsx` skeleton visible and stuck forever, the real page hidden inside `<div hidden id="S:0">` with its mount effects never firing. Client-side nav unaffected. Dev AND `next build && next start`.

**Root cause (not a hydration mismatch):** React 19's streamed-Suspense client runtime schedules both the fallback→content swap (`$RV`, queued by the inline `$RC("B:0","S:0")` completion script) and the boundary hydration retry (`_reactRetry`) through `requestAnimationFrame`. rAF never fires while `document.visibilityState === "hidden"` — and the embedded QA browser pane is permanently hidden — so the boundary reveal and the page's hydration starve forever. Diagnosed live: boundary comment marker stuck at `$~` (completed, reveal queued), `$RB.length === 2`, layout effects running (layout sits outside the boundary) but zero page fetches; manually calling `$RV($RB)` merged the DOM but the page still never hydrated because `_reactRetry` is also rAF-scheduled.

**Fix:** `apps/web/src/lib/hidden-tab-raf.ts` — pre-hydration inline boot script in the root layout that services `requestAnimationFrame` via `setTimeout(32ms)` while the document is hidden (native rAF untouched when visible; negated timeout ids so `cancelAnimationFrame` can route). **Second bug found en route:** production CSP is enforced (dev is report-only) with per-request nonces, and our handwritten inline `<script dangerouslySetInnerHTML>` tags carried no nonce — so in every prod deploy the theme boot script (and initially this fix) was silently blocked. Root layout now reads middleware's `x-nonce` and stamps both inline scripts.

- Verified hard loads of `/ar-PS/notifications`, `/ar-PS/jobs`, `/ar-PS/feed` in dev (:3100) and prod build (:3001), all in a hidden tab: single visible `<main>`, live data fetched, SSE "مباشر" badge up. Notifications/jobs interactivity on hard load also needs PR #65's `getValidAccessToken` bootstrap (kept; verified with it merged locally).
- §10 S5 key-warning "regression" was a false alarm: an artifact of the pre-fix zombie state. Post-fix, hard loads of feed/notifications/jobs log zero key warnings; the S5 div wrapper is intact and sufficient.
- Real-user impact was mild (visible tabs reveal on next frame; background tabs recover on focus) — but headless/automated QA, and anything else that loads while hidden, hard-wedged forever.
- Gate: prettier, lint 8/8, type-check 13/13, tests 11/11 tasks green, qa:design, lint:tokens, release-placeholders, Playwright a11y.

## 15. Ponytail audit 6 — dedupe + NativeWind corpse (2026-07-25)

Sixth repo-wide over-engineering pass. Gate green: prettier, lint 8/8, type-check 13/13, tests 11/11 tasks (mobile 35 suites / 100 tests), lint:tokens, qa:design, release-placeholders, `mobile:recovery-check`.

- **NativeWind's corpse, fully removed.** Mobile has no babel plugin, no `withNativeWind` in metro, no `tailwind.config.*`, and zero `className=` in its source — NativeWind was pulled long ago but three artifacts survived it: `packages/config/tailwind-preset.js` (92 lines of hand-mirrored token hexes), the ~20-line "preset drift guard" in `lint-tokens.mjs` that existed _only_ to police that mirror, and the `react-native-css-interop` dep. All deleted, plus the `qa:design` colour allowlist entry, the `files` entry, and the `index.js` pointer. README + project-spec now say "RN StyleSheet" instead of advertising NativeWind.
- **`design-handoff-2026-05/` — duplicates cut, gate docs kept.** §12 deferred archiving this tree ("83 doc references, churn > value"); that call still holds for the tree as a whole, but the _copies_ inside it were free to take: `05-prototype/` was **byte-identical** to `docs/_archive/prototype-2025/` (verified `diff -r`, 75,193 bytes each), and `02-system/` + `01-brand/BRAND.md` + `06-fixtures/tokens-quickref.css` were stale forks of `DESIGN.md` / `docs/design/*.md` / the token source — a drifted copy of the design source of truth is worse than no copy. `07-audits/` was regenerable output. Kept: `10-ask.md` (**Pass 2 ask still awaiting lead approval**), `08-pain.md`, `08-problems.md`, `09-moodboard/`, `06-fixtures/content.json`, and the frozen status docs. `00-README.md` rewritten to point at live sources.
- **`scripts/check-duplicate-imports.mjs` deleted** (57 lines of regex import parsing). `tsc` already emits `TS2300: Duplicate identifier` for exactly this — verified against a repro. It wasn't even in CI, only in `mobile:recovery-check`, which still passes without it.
- **Dangling doc pointers repaired:** four web pages cited `design-handoff-2026-05/PRODUCT-HEALTH.md`, a path that never existed in that dir → repointed at `docs/design/PRODUCT-HEALTH-2026-05-16.md`. Three ui-web atoms cited `MISSING-ELEMENTS.md`, deleted by an earlier audit → citation dropped.
- **`shadowStyle` unexported** from the ui-native barrel (package-internal; `ShadowKind` stays public).
- **Checked, NOT cut:** `qa-design.mjs`'s `legacyOversizeAllowlist` — all 11 entries verified still >300 LOC, and the rule still blocks _new_ oversized files. It's a working ratchet, not dead config; deleting it would trade a guard for 13 lines. Twin web/mobile `safety.ts`/`notifications.ts` (~145 dup lines) re-confirmed as cheaper than the injection abstraction, per §12.
- **Noted, not actioned:** `design-handoff-2026-06/missing-flows/Baydar Missing Flows (standalone).html` is 1,015,102 bytes in git — every clone pays it. It's current design reference, so it stays until Pass 2 closes.
- **Architecture verdict:** zero `abstract class`, zero `interface I*`, no real factories, no committed `dist/`. The Nest modules that read as boilerplate are framework-required. Remaining bloat is docs, not code.
