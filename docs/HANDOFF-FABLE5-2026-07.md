...
- **Still open (priority order):** billing review DTO enrichment (user/company still render as cuids on `/billing` — same treatment as moderation: name/handle via API include); dev-only React key warning logged against `AppShell` (seen once in console during QA, no unkeyed map found in AppShell/Nav — reproduce before chasing); Sentry wiring (sharper finding: only mobile has a Sentry SDK — the api merely _validates_ `SENTRY_DSN` in `env.ts` and never initializes an SDK, web has PostHog only; adding `@sentry/node` + `@sentry/nextjs` is an owner dep decision); §4 env/ops blockers (owner accounts).
