...
| Native `Tabs` visuals              | DESIGN.md §6.3 specifies a `brand-600` underline with an `ink` label. Native still draws the filled pill strip it drew as `SegmentedControl`. The API converged first; moving the pixels is five screens and wants device evidence.                                      |
