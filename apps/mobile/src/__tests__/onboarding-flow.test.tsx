  ...
  useTranslation: () => ({
    ...
    t: (key: string, values?: Record<string, unknown>) => {
      const translations: Record<string, string> = {
        ...
        "onboarding.network.emptyTitle": "لا توجد اقتراحات الآن.",
        "onboarding.network.emptyBody": "ابنِ شبكتك لاحقًا.",
...
describe("OnboardingScreen", () => {
  ...
  it("saves profile setup, work history, and enters the feed", async () => {
    ...
    expect(
      mockApiFetch.mock.calls.filter(([path]) => path === "/connections/suggestions?limit=8"),
    ).toHaveLength(1);
