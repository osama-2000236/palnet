...
export default function JobsPageRoute(): JSX.Element {
  ...
  return (
    <div className="mx-auto grid w-full max-w-chrome grid-cols-1 gap-6 px-4 py-6 lg:grid-cols-[260px_minmax(0,1fr)]">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-3">
              ...
              {job.viewer.hasApplied ? (
                <span className="border-success/30 bg-success/10 text-success shrink-0 rounded-full border px-2 py-0.5 text-nav font-semibold">
                  {t("appliedBadge")}
                </span>
              ) : null}
            </div>
            ...
            {job.skillsRequired.length > 0 ? (
              <ul className="mt-2 flex flex-wrap gap-1">
                {job.skillsRequired.slice(0, 5).map((s) => (
                  <li
                    key={s}
                    className="bg-surface-subtle text-ink-muted rounded-full px-2 py-0.5 text-nav"
                  >
                    {s}
                  </li>
                ))}
