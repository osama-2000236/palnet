...
/**
 * The plan catalog. Lives here because two independent writers need it: the API's
 * `ensurePlan` (which upserts on first read) and `packages/db/prisma/seed.ts`.
 * They held byte-identical copies until they drifted apart on someone's watch.
 */
